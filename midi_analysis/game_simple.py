...
class DynamicMusicSheet:
    def __init__(self):
        ...
        self.velocity_tolerance_input_active = False
        self.velocity_tolerance_text = ''
    ...
    def setup_ui_elements(self):
        ...
        self.time_tolerance_button_rect = pygame.Rect(400, 20, 230, 40)  # Time Tolerance button
        self.time_tolerance_input_rect = pygame.Rect(650, 20, 100, 40)  # Input box for time tolerance
        self.velocity_tolerance_button_rect = pygame.Rect(800, 20, 230, 40)  # Time Tolerance button
        self.velocity_tolerance_input_rect = pygame.Rect(1050, 20, 100, 40)  # Input box for time tolerance
    ...
    def draw_legends(self):
        ...
        legend_width = 180
        start_y = 500
        start_x = self.screen_width - legend_width - 30
    ...
    def run(self):
            ...
            while running:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.record_button_rect.collidepoint(event.pos):
                            ...
                            self.bpm_text = ''
                        elif self.time_tolerance_button_rect.collidepoint(event.pos):  # 点击时间容差按钮
                            self.time_tolerance_input_active = True
                            self.time_tolerance_text = ''  # 清空输入框内容
                        elif self.velocity_tolerance_button_rect.collidepoint(event.pos):
                            self.velocity_tolerance_input_active = True
                            self.velocity_tolerance_text = ''
                        ...
                        if self.time_tolerance_input_active:  # Time Tolerance 输入框逻辑
                            if event.key == pygame.K_RETURN:
                                try:
                        ...
                        if self.velocity_tolerance_input_active:  # Time Tolerance 输入框逻辑
                            if event.key == pygame.K_RETURN:
                                try:
                                    new_velocity_tolerance = float(self.velocity_tolerance_text)
                                    self.velocity_tolerance = new_velocity_tolerance
                                    self.velocity_tolerance_input_active = False
                                except ValueError:
                                    pass
                            elif event.key == pygame.K_BACKSPACE:
                                self.velocity_tolerance_text = self.velocity_tolerance_text[:-1]
                            else:
                                self.velocity_tolerance_text += event.unicode
                ...
                self.screen.blit(bpm_label_text, bpm_label_rect.move(-115, 500+17))

                # Time Tolerance Button
                time_tolerance_active = self.time_tolerance_button_rect.collidepoint(mouse_pos)
                self.draw_button_with_shadow(self.screen, self.time_tolerance_button_rect, "Set Time Tolerance", self.font_title, active=time_tolerance_active)

                # Time Tolerance Button
                velocity_tolerance_active = self.velocity_tolerance_button_rect.collidepoint(mouse_pos)
                self.draw_button_with_shadow(self.screen, self.velocity_tolerance_button_rect, "Set Velo Tolerance", self.font_title, active=velocity_tolerance_active)

                # Draw the current time_tolerance label below the button
                velocity_tolerance_label_rect = self.velocity_tolerance_button_rect.move(0, 50)
                velocity_tolerance_label_text = self.font_title.render(f"Velo Tolerance: {self.velocity_tolerance:.2f}s", True, (0, 0, 0))
                self.screen.blit(velocity_tolerance_label_text, velocity_tolerance_label_rect.move(-775, 500+80))

                if self.velocity_tolerance_input_active:
                    pygame.draw.rect(self.screen, (255, 255, 255), self.velocity_tolerance_input_rect)
                    tolerrance_surface = self.font_title.render(self.velocity_tolerance_text or '', True, (0, 0, 0))
                    self.screen.blit(tolerrance_surface, self.velocity_tolerance_input_rect.move(10, 10))


                if self.time_tolerance_input_active:
                    pygame.draw.rect(self.screen, (255, 255, 255), self.time_tolerance_input_rect)
                    tolerance_surface = self.font_title.render(self.time_tolerance_text or '', True, (0, 0, 0))
                    self.screen.blit(tolerance_surface, self.time_tolerance_input_rect.move(10, 10))
                ...
                # Draw the current time_tolerance label below the button
                time_tolerance_label_rect = self.time_tolerance_button_rect.move(0, 50)
                time_tolerance_label_text = self.font_title.render(f"Tolerance: {self.time_tolerance:.2f}s", True, (0, 0, 0))
                self.screen.blit(time_tolerance_label_text, time_tolerance_label_rect.move(-375, 500+50))
